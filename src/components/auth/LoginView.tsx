              ...
              <div className="grid sm:grid-cols-2 gap-3">
                {roleInfo.map((item) => (
                  <button
                    ...
                    className={`p-3 bg-slate-900/90 border border-slate-800 rounded-xl text-left transition-all group flex items-start gap-3 ${item.color}`}
                  >
                    <img
                      src={item.avatarUrl}
                      alt={item.name}
                      ...
                      onError={(e) => {
                        (e.target as HTMLImageElement).src = `https://picsum.photos/seed/${item.role}/150/150`;
                      }}
                      className="w-10 h-10 rounded-xl object-cover border border-slate-700 shadow shrink-0 group-hover:border-amber-400 transition-colors"
                    />
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between gap-1">
                        ...
                        <span className={`text-[9px] font-mono font-bold px-1.5 py-0.5 rounded border shrink-0 ${item.badgeBg}`}>
                          {item.role}
                        </span>
                      </div>
                      <div className="text-[11px] font-medium text-slate-300 mt-0.5 truncate">{item.name}</div>
